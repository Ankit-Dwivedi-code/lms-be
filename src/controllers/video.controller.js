...
const publishAVideo = asyncHandler(async (req, res) => {
    const { courseName, title, description, youtubeLink } = req.body;

    if (!courseName || !title || !description || !youtubeLink) {
        throw new ApiError(400, "Course name, title, description, and YouTube link are required!");
    }
    ...
    // Add the video ID to the course's courseVideos array
    course.courseVideos.push(video._id);
    await course.save();

    return res
        .status(200)
        .json(new ApiResponse(200, video, "Video added to course successfully"));
});
